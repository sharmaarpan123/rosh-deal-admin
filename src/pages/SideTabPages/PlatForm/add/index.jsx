...
const AddPlatForm = () => {
  ...
  const submitHandler = catchAsync(async (data) => {
    ...
    if (id) {
      ...
      res = await ADD_PLATFORM({ ...data, ...(image && { image }) });
    }
    checkResponse({
      res,
      showSuccess: true,
  ...
  const getData = catchAsync(async () => {
    const res = await PLATFORM_BY_ID(id);
    const success = checkResponse({ res, setData: setPlatFormDetails });
              ...
              >
                <Form onSubmit={handleSubmit(submitHandler)}>
                  <Row className="d-flex justify-content-center">
                    <Col lg="12" className="my-2">
                      <div
                        className="mx-auto position-relative upload text-center  rounded rounded-circle"
                        style={{
