...
const optionObjectSchema = (keyName) =>
...
const schema = z
  .object({
    ...
    dealCategory: optionObjectSchema("dealCategory"),
    deal: optionObjectSchema("deal"),
    redirectUrl: z
      .string()
      .refine(
        (data) => {
          if (data) {
            if (!isUrlValid(data)) return false;
          }
          return true;
        },
  ...
  .refine(
    (data) => {
      if (
        data.posterType.value === POSTER_ENUM.REDIRECT &&
        !data?.redirectUrl
      ) {
        return false;
      }
  ...
  .refine(
    (data) => {
      ...
      if (data.posterType.value === POSTER_ENUM.DEAL && !data?.deal?.value) {
        return false;
      }
      return true;
    },
    {
      message: "please add Deal Id",
      path: ["deal"],
    }
  )
  .refine(
    (data) => {
      if (
        data.posterType.value === POSTER_ENUM.DEALCATEGORY &&
        !data?.dealCategory?.value
      ) {
        return false;
      }
      return true;
    ...
    {
      message: "please add  Deal Category Id",
      path: ["dealCategory"],
    }
  )
  .refine(
    (data) => {
      if (data.posterType.value === POSTER_ENUM.BRAND && !data?.brand?.value) {
        return false;
      }
      return true;
    },
    {
      message: "please add Brand Id",
      path: ["brand"],
    }
  );
...
const AddEditPoster = () => {
  ...
  const {
    ...
    values: {
      ...
      title: data?.title || "",
      image: data?.image || "",
      posterType: data?.posterType || emptyOption,
      brand: data?.brand || emptyOption,
      dealCategory: data?.dealCategory || emptyOption,
      redirectUrl: data?.redirectUrl || "",
      deal: data?.deal || emptyOption,
    },
  ...
  const submitHandler = catchAsync(async (data) => {
    ...
    const body = {
      ...
      deal: data?.deal?.value,
      posterType: data?.posterType?.value,
    };

    if (id) {
      res = await POSTER_EDIT(body);
    } else {
      res = await POSTER_ADD(body);
    }
    checkResponse({
      res,
      showSuccess: true,
  ...
  const getData = catchAsync(async () => {
    ...
    checkResponse({
      res: res[0],
      setData: (data) => setBrandOptions((p) => makeOptions(data, "name")),
    });
    checkResponse({
      res: res[1],
      setData: (data) =>
        setDealsOptions((p) => makeOptions(data, "productName")),
    });
    checkResponse({
      res: res[2],
      setData: (data) =>
                ...
                <Form onSubmit={handleSubmit(submitHandler)}>
                  <Row className="d-flex justify-content-center">
                    <Col lg="12" className="my-2">
                      ...
                      {errors.image && (
                        <p className="text-danger w-100 text-center">
                    ...
                    <Col lg="6" md="12" className="my-2">
                      <div className="py-2">
                        ...
                        <Controller
                          ...
                          name="posterType"
                          render={({ field }) => (
                            <Select {...field} options={posterEnumOptions} />
                          )}
                        />
                        {errors?.posterType && (
                          <p className="text-danger m-0">
